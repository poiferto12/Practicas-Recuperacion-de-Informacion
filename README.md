# UDC - Recuperación de Información - Assignment

Template for the assignment of the Information Retrieval course - University of A Coruña (UDC)

The template includes code examples for reading CSV and JSON files. The structure of the CSV and JSON files is similar to the assigment.
...
Each row in the CSV file is read into a record [`Movies`](src/main/java/es/udc/fi/irudc/pri/movies/Movie.java) with the fields we are interested from the CSV. The record definition uses some Java annotations defined by Jackson to instruct the library how to map the fields from the file to the attributes of the class. These annotations are:
